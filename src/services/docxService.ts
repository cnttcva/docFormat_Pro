...
export const processDocx = async (file: File, options: DocxOptions, dictionary: any[] = []): Promise<ProcessResult> => {
  ...
  try {
    ...
    const cmToTwips = (cm: number): string => {
      return String(Math.round(cm * TWIPS_PER_CM));
    };

    const forceFinalPageMargins = (doc: Document, opts: any): void => {
