...
import { insertSpecialDocumentSignatureBlock } from './specialDocuments';
...
import {
  cmToTwips,
  forceFinalPageMargins,
  getMarginCm,
  ensureDefaultHeaderReferenceFirst,
  normalizeSectPrChildOrder,
  ensureFinalBodySectPr,
  removeChildrenByLocalName
} from './docx/docSection';
import {
  ensureEveryTableCellEndsWithParagraph,
  normalizeTableXmlForWord,
  reorderElementChildrenByLocalName
} from './docx/docRepairGuards';
import { removeAutomaticNumbering } from './docx/docNumbering';
...
splitParagraphsByLineBreaks(doc, body);
    ...
    if (finalOptions.headerType === HeaderType.PARTY) {
        const topElements = Array.from(body.childNodes).slice(0, 15);
        for (const node of topElements) {
            if (node.nodeType !== 1) continue;
            const el = node as Element;
            const nodeName = el.nodeName.replace("w:", "");
            if (nodeName !== "tbl" && nodeName !== "p") continue;

            const text = el.textContent?.toUpperCase() || "";
            const cleanText = text.replace(/^[\s\xA0]+/, ''); // Loại bỏ khoảng trắng đầu dòng

            // Chạm đến Tiêu đề chính (NGHỊ QUYẾT, QUYẾT ĐỊNH...) thì dừng lại để không xóa lẹm nội dung
            const isMainTitle = DOC_TYPE_KEYWORDS.some(k => cleanText.replace(/^[^A-ZÀ-Ỹ]+/, '').startsWith(k));
            if (isMainTitle && cleanText.length < 100) break;

            // Nhận diện các dấu hiệu của Header Đảng cũ để tiêu diệt
            let isArtifact = false;
            if (nodeName === "tbl" && cleanText.includes("ĐẢNG CỘNG SẢN")) {
                isArtifact = true;
            } else if (nodeName === "p") {
                if (
                    cleanText.startsWith("ĐẢNG CỘNG SẢN") ||
                    cleanText.startsWith("ĐẢNG BỘ") ||
                    cleanText.startsWith("CHI BỘ") ||
                    /^SỐ[:\s]+.*-/i.test(cleanText) ||
                    (cleanText.includes("NGÀY") && cleanText.includes("THÁNG") && cleanText.includes("NĂM") && cleanText.length < 80 && !cleanText.startsWith("KÍNH GỬI"))
                ) {
                    isArtifact = true;
                }
            }
...
// ==========================================
await removeAutomaticNumbering(doc, zip, parser, finalOptions);
    ...
    // Văn bản Đảng: nhận diện thống nhất dòng phân cách và đoạn mở đầu thân văn bản.
    // Mục tiêu: dòng "-----" kết thúc khối trích yếu; đoạn "Ngày ... tháng ... năm ..." không được coi là trích yếu.
    const normalizeInlineTextForDetect = (text: string) => {
        return (text || "")
            .replace(/[‐-‒–—―]/g, "-")
            .replace(/\s+/g, " ")
            .trim();
    ...
    const isDashOnlyLine = (text: string) => {
        const compact = normalizeInlineTextForDetect(text).replace(/\s+/g, "");
        return /^[-_*]{3,}$/.test(compact);
    };

    const isPartyBodyOpeningLine = (text: string) => {
        const t = normalizeInlineTextForDetect(text);
        ...
        if (matchedKeyword) {
            ...
            while (currentIndex < paragraphs.length && linesCaptured < 8) {
                ...
                if (tempText.length > 0) {
                    ...
                    // Văn bản Đảng: dòng "-----" là ranh giới kết thúc trích yếu.
                    // Xóa dòng gạch cũ để bên dưới chỉ chèn lại một dòng chuẩn duy nhất.
                    if (finalOptions.headerType === HeaderType.PARTY && isDashOnlyLine(tempText)) {
                        tempP.parentNode?.removeChild(tempP);
                        currentIndex++;
                        break;
                    }

                    // Văn bản Đảng: đoạn mở đầu dạng "Ngày ... tháng ... năm ..." là thân văn bản,
                    // không phải dòng trích yếu/trích dẫn.
                    if (finalOptions.headerType === HeaderType.PARTY && isPartyBodyOpeningLine(tempText)) {
                        break;
                    }
    ...
    if (finalOptions.headerType !== HeaderType.NONE && body) {
        ...
        /**
         * BƯỚC 3 - ĐIỀU PHỐI VĂN BẢN ĐẶC BIỆT
         *
         * Nếu là Quyết định hành chính nhà trường:
         * - module specialDocuments/quyetDinhNT.ts sẽ tự chèn Nơi nhận + chữ ký
         * - nếu có văn bản kèm theo, chữ ký được chèn trước phần kèm theo
         * - không dùng luồng chữ ký mặc định của docSignature.ts
         *
         * Nếu không phải văn bản đặc biệt:
         * - giữ nguyên luồng cũ bằng createSignatureBlock(...)
         */
        const handledBySpecialDocument = insertSpecialDocumentSignatureBlock(
          doc,
          finalOptions as any,
          detectedDocType
        );

        if (!handledBySpecialDocument) {
    ...
    /**
     * Với Quyết định hành chính nhà trường, phần Nơi nhận đã được module riêng
     * src/services/specialDocuments/quyetDinhNT.ts tạo khóa cứng.
     *
     * Không cho bộ auto-format Nơi nhận mặc định bên dưới can thiệp nữa,
    ...
    for (const p of allFinalParagraphs) {
        ...
        if (inNoiNhanZone) {
            ...
            if (fullText.trim().length > 0 && fullText.trim().length < 150) {
                ...
                if (finalOptions.headerType === HeaderType.PARTY) {
                    const normalizedReceiver = cleanText
                      .replace(/[.,;]+$/g, "")
                      .replace(/\s+/g, " ")
                      .trim()
                      .toLowerCase();

                    // Văn bản Đảng không dùng dòng mặc định kiểu văn bản hành chính.
                    // Nếu "Như trên" đã bị module khác thêm vào trước đó thì xóa khỏi document.
                    if (
                      normalizedReceiver === "như trên" ||
                      normalizedReceiver === "nhu tren"
                    ) {
                      p.parentNode?.removeChild(p);
                      continue;
                    }
                    ...
                    // Chặn lỗi áp mẫu hành chính sang văn bản Đảng.
                    // Nghị quyết chi bộ dùng "Lưu: Chi bộ.", không ép thành "Lưu: VT."
                    if (/^lưu\s*:\s*vt\.?$/i.test(formattedText) || /^luu\s*:\s*vt\.?$/i.test(formattedText)) {
                      formattedText = "Lưu: Chi bộ.";
                    }
