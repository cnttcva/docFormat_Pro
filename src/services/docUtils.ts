...
export const smartNormalizeText = (text: string): string => {
    ...
    return t
        .split(/\s+/)
        .map((word, index) => {
            const rawWord = word.normalize('NFC');

            const leading = rawWord.match(/^[^\p{L}\p{N}]+/u)?.[0] || '';
            const trailing = rawWord.match(/[^\p{L}\p{N}]+$/u)?.[0] || '';
            const core = rawWord.slice(
                leading.length,
                rawWord.length - trailing.length
            );
            ...
            const lowerCore = core.toLocaleLowerCase('vi-VN');

            if (index === 0) {
                return (
                    leading +
                    lowerCore.charAt(0).toLocaleUpperCase('vi-VN') +
                    lowerCore.slice(1) +
                    trailing
                );
            }
