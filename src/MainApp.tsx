...
export default function MainApp() {
  ...
  const isLicensed = authStatus === 'REGISTERED';
  const accessStatus = getAccessStatus(isLicensed);
  const canUseApp = accessStatus.canUse;
  const isTrialMode = accessStatus.mode === 'trial';
  ...
  const handleProcess = async () => {
    ...
    const latestAccess = getAccessStatus(authStatus === 'REGISTERED');

    if (!latestAccess.canUse) {
      setTrialNotice(
        latestAccess.message ||
          'Bạn đã hết lượt dùng thử. Vui lòng đăng ký bản quyền chính thức để tiếp tục sử dụng docFormat Pro.'
      );
...
setTrialRemaining(nextRemaining);
  ...
  options.headerType === HeaderType.DEPARTMENT;
  const hasSelectedDocSymbol = Boolean(options.docSymbol);
  return (
  ...
  Gia hạn bản quyền
        ...
        <div
          className={`bg-white/80 backdrop-blur-xl rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white overflow-hidden relative group ${
            !canUseApp ? 'opacity-50 pointer-events-none grayscale' : ''
          }`}
        >
              ...
              <div className="space-y-6 animate-fadeIn">
                ...
                <div className="bg-slate-50/50 rounded-3xl p-2 border border-dashed border-slate-200 hover:border-violet-300 hover:bg-violet-50/30 transition-all duration-300">
                  <Dropzone
                    ...
                    disabled={isUploadDisabled || !canUseApp || !hasSelectedHeaderType || !hasSelectedDocSymbol}
                  />
                </div>
              </div>
            )}
